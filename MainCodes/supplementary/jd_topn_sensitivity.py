"""
# NOTE (Aug 2026): per-day SKU filter unified at 50 (matching Section 7);
# FI benchmark computed by the exact MCI scalar representation (Prop. 5),
# which is available on every day at every N.
...
def scalar_fi_point(c, alpha, beta, M):
    """Exact FI fixed point under MCI via the one-scalar representation."""
    import numpy as _np
    def _sh(p):
        A = alpha * _np.power(p, -beta); return A/(1.0+A.sum())
    def g(A):
        p = (c + A)/(1.0 - 1.0/beta); s = _sh(p)
        return float(s @ (p - c)) - A
    hi = 1.0
    while g(hi) > 0 and hi < 1e7: hi *= 4.0
    A = _brentq(g, 0.0, hi, xtol=1e-12, maxiter=300)
...
def run_pricing_at_N(agg, M, sku_to_beta, N):
    ...
    for d in days:
        ...
        try:
            p_ms = scalar_fi_point(c, alpha, beta_vec, M)
            it_ms, t_ms = -1, np.nan
        ...
        try:
            ...
            pi_unif = np.nan
        try:
            p_bn = scalar_fi_point(c, alpha, beta_vec, M)
            it_bn, t_bn = -1, np.nan
