...
N, R = 60, 80
SIGMAS = [0.05, 0.15, 0.25, 0.35, 0.45, 0.60, 0.75, 0.90]
MKTS = 12
...
for sg in SIGMAS:
    for m in range(MKTS):
        ...
        S_target = rng.uniform(0.15, 0.40)
        shift = -4.0
...
d = pd.DataFrame(rows)
d.to_csv("/sessions/epic-brave-gauss/mnt/outputs/regret_misspec.csv", index=False)
