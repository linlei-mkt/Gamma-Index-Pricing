...
calibrated Monte Carlo, and the BLP supply-side identification
implications are in **Appendices M, N, and O** respectively.
Appendices A–L contain the proofs of all theorems, propositions,
and corollaries, plus the Lipschitz bound for the $\gamma$-update
map.
...
| §8 (Tables 4–5, Figures 3–4) | GMV-constrained pricing on JD; floor-multiplier sensitivity | `jd_gmv_constrained.py`, `jd_gmv_floor_sensitivity.py` |
| App M.1 (Table 6) | Hausman-IV from cross-DC variation | `jd_hausman_iv.py` |
| App M.2 (Table 7, Figure 5) | Wall-clock scalability at $n \in \{500,\ldots,50{,}000\}$ | `scalability_demo.py` |
| App M.3 (Table 8) | $M_{\mathrm{mult}} \times c/\bar p$ sensitivity grid | `jd_hb_sensitivity.py` |
| App M.4 | Mixed-logit Monte Carlo robustness (narrative only) | `mixed_logit_robustness.py` |
| App M.5 (Figure 6) | Empirical local operator-norm contraction diagnostic | `jd_cbar_diagnostic.py` |
| App M.6 (Figures 7–8) | Posterior predictive check, holdout, uncertainty propagation | `jd_hb_validation.py` |
| App M.7 (Table 9, Figure 9) | Top-$N$ catalog-truncation robustness | `jd_topn_sensitivity.py` |
| App M.8 (Figures 10–11) | $\gamma^\star$-misspecification sweep + outcome-based calibration | `jd_gamma_robustness.py` |
| App N (Table 10) | Calibrated Monte Carlo + HKMR (1995) illustration | `empirical_gamma.py` |
| App O | BLP supply-side identification under revenue constraints | (analytical, no script) |
