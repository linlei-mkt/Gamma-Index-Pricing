...
> Round-7 note (2026-08-21): the constrained comparison now uses a common feasible set — the cost safeguard (p ≥ 1.0001c) is removed from the γ-iteration, so both γ and scalar-FI may price below cost (γ: 582/10,408 pairs at φ=1.15; FI: 254). Revenue along the γ-path is hump-shaped once below-cost prices are allowed; the outer search brackets the least-negative crossing (`round7_common_feasible.py`; Table 4 / Fig. jd-gmv / App M.1 superseded by `round8_sync.py` in round 8 — round 7 remains canonical for the β-floor sensitivity only). New headline: γ-tuned median gap 5.36% (30/31 feasible; March 1 infeasible, hump peak = 98.3% of target), uniform 15.93%, mean advantage 9.9pp. β-floor sensitivity in `round7_beta_floor_sens.csv` (unconstrained gap 1.60–1.61% across floors 1.05–1.5). Constrained posterior propagation rerun: 4.5% median, 2.3pp width (`round7_constrained_posterior_prop.csv`). Prop 5(i) prior art now cited: Gallego–Wang (2014), Li–Huh (2011), Keller–Levi–Perakis (2014).
...
> Exploratory (not referenced in the paper): `gamma_capacity.py` computes the GMV capacity of the single-scalar pricing families (sup of revenue along the γ and uniform paths vs. the unrestricted revenue maximum). Outputs `gamma_capacity.csv` and `figures/gamma_capacity.png`. Held in reserve for a potential revision request to formalize the March-1 infeasibility.
